...
export function usePendingPoints(vaultAddress: string, source: string) {
  ...
  return useMemo(() => {
    ...
    if (
      typeof globalIndex !== "bigint" ||
      typeof userIdx !== "bigint" ||
...
export function useEarningRate(vaultAddress: string, source: string) {
  ...
  return useMemo(() => {
    ...
    const pointsPerDayPerShare =
      (baseRatePerSec * multiplier * BigInt(86400)) / BigInt(1e18);

    const sharesPerETH =
      totalSupply && totalAssets
        ? (totalSupply * BigInt(1e18)) / totalAssets
        : BigInt(1);
    ...
    return {
      ...
      isBootsted: multiplier > BigInt(1e18),
    };
...
export function useAllVaultsPoints() {
  ...
  return useMemo(() => {
    const vault1Points =
      Array.isArray(vault1Preview) && vault1Preview[0]
        ? vault1Preview[0]
        : BigInt(0);
    const vault2Points =
      Array.isArray(vault2Preview) && vault2Preview[0]
        ? vault2Preview[0]
        : BigInt(0);
    const vault3Points =
      Array.isArray(vault3Preview) && vault3Preview[0]
        ? vault3Preview[0]
        : BigInt(0);

    const vault1Sources =
      Array.isArray(vault1Preview) && Array.isArray(vault1Preview[1])
        ? vault1Preview[1].map((src: string, index: number) => ({
            source: src,
            label: getSourceLabel(src),
            points:
              Array.isArray(vault1Preview[2]) && vault1Preview[2][index]
                ? vault1Preview[2][index]
                : BigInt(0),
          }))
        : [];

    const vault2Sources =
      Array.isArray(vault2Preview) && Array.isArray(vault2Preview[1])
        ? vault2Preview[1].map((src: string, index: number) => ({
            source: src,
            label: getSourceLabel(src),
            points:
              Array.isArray(vault2Preview[2]) && vault2Preview[2][index]
                ? vault2Preview[2][index]
                : BigInt(0),
          }))
        : [];

    const vault3Sources =
      Array.isArray(vault3Preview) && Array.isArray(vault3Preview[1])
        ? vault3Preview[1].map((src: string, index: number) => ({
            source: src,
            label: getSourceLabel(src),
            points:
              Array.isArray(vault3Preview[2]) && vault3Preview[2][index]
                ? vault3Preview[2][index]
                : BigInt(0),
          }))
        : [];
...
export function getVaultKeyByAddress(
  address: string
): keyof typeof VAULT_ADDRESSES | null {
  const normalizedAddress = address.toLowerCase();

  if (normalizedAddress === VAULT_ADDRESSES.vault1.toLowerCase())
    return "vault1";
  if (normalizedAddress === VAULT_ADDRESSES.vault2.toLowerCase())
    return "vault2";
  if (normalizedAddress === VAULT_ADDRESSES.vault3.toLowerCase())
    return "vault3";
